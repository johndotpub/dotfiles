...
#  2) Existing ~/.tmux.conf is preserved by default.
#  3) --override backs up and replaces ~/.tmux.conf with expected symlink.
...
# Scenario 2: preserve existing ~/.tmux.conf when not overriding.
HOME_TWO="${TMP_DIR}/home-two"
mkdir -p "$HOME_TWO"
printf '%s\n' '# keep-my-tmux-conf' > "${HOME_TWO}/.tmux.conf"
run_install_for_home "$HOME_TWO"
grep -Fxq "# keep-my-tmux-conf" "${HOME_TWO}/.tmux.conf"
assert_no_backups "$HOME_TWO"

# Scenario 3: --override replaces ~/.tmux.conf and creates backup.
HOME_THREE="${TMP_DIR}/home-three"
mkdir -p "$HOME_THREE"
printf '%s\n' '# replace-me-tmux-conf' > "${HOME_THREE}/.tmux.conf"
run_install_for_home "$HOME_THREE" --override
test -L "${HOME_THREE}/.tmux.conf"
if ! compgen -G "${HOME_THREE}/.tmux.conf.bak.*" >/dev/null; then
  echo "Expected ~/.tmux.conf backup not found in override scenario." >&2
  exit 1
fi
